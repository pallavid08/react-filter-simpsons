...
const App = () => {
  ...
  const handleSimpsonsOnlyClick = () => {
    setShowSimpsonsOnly(!showSimpsonsOnly);
  };

  return (
    <div>
      <button onClick={handleSimpsonsOnlyClick}>
        Simpsons Family Only: {showSimpsonsOnly ? 'ON' : 'OFF'}
      </button>
      {quoteList
        .filter(
          (quote) => !showSimpsonsOnly || quote.character.includes('Simpson')
        )
        .map((quote, index) => (
